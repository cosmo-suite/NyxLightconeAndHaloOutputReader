...
int main(int argc, char** argv) {
    ...
    std::string lightcones_dir;

    // Parse command-line arguments
    for (int i = 1; i < argc; ++i) {
        std::string arg = argv[i];
        std::string prefix = "--lightcones_dir=";
        if (arg.substr(0, prefix.size()) == prefix) {
            lightcones_dir = arg.substr(prefix.size());
        }
    }

    // Check if argument was provided
    if (lightcones_dir.empty()) {
        if (rank == 0)
            std::cerr << "Error: --lightcones_dir argument is required.\n";
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    // Check if lightcones_dir exists
    if (!std::filesystem::exists(lightcones_dir)) {
        if (rank == 0)
            std::cerr << "Error: Directory does not exist: " << lightcones_dir << "\n";
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    if (rank == 0) {
        std::cout << "Using lightcones_dir: " << lightcones_dir << std::endl;
    }
    ...
    for (auto& p : fs::directory_iterator(lightcones_dir)) {
